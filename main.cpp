...
void endload(){
    ...
    for(size_t i =0; i<banner.size(); i++){
        mvwprintw(load, (ventana.y/2)-(banner.size()/2)+i, (ventana.x/2)-(banner[1].size()/2),"\t\t\t\t\t\t\t\t\t\t", banner[i].c_str());
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
        wrefresh(load);
    }
    wrefresh(load);
    std::string finaliza = "ha finalizado el programa";
    mvwprintw(load,(ventana.y/2)-1, (ventana.x/2)-(finaliza.size()/2), "%s", finaliza.c_str() );
    wrefresh(load);
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    delwin(
    load);
